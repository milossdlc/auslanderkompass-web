...
function DeadlineCard({ item, lang, dispatch, authority }: { item: SmartDeadline; lang: Lang; dispatch: Dispatch<Action>; authority: ReturnType<typeof getLocalAuthority> }) {
  ...
  return <article className={`smart-deadline-card ${item.kind} ${urgent ? "urgent" : ""}`}>
    <div className="smart-deadline-top"><div className={`deadline-kind ${item.kind}`}><span className="deadline-kind-icon"><Icon name={item.kind === "legal" ? "flag" : "calendar"} size={17}/></span><span>{item.kind === "legal" ? C.legal : C.recommended}</span></div><span className={`deadline-countdown ${urgent ? "urgent" : ""}`}>{signed < 0 ? C.overdueDays(Math.abs(signed)) : C.days(signed)}</span></div>
    <div className="smart-deadline-date disp">{fmtDate(item.date, lang)}</div><h3>{item.title}</h3><p>{item.description}</p>
    <div className="smart-authority"><div className="smart-authority-label"><Icon name="building" size={16}/><span>{C.authority}</span></div><strong>{authority.name}</strong><small>{authority.description}</small><a href={authority.url} target="_blank" rel="noopener noreferrer">{C.source} →</a></div>
    <div className="smart-deadline-meta"><a href={item.sourceUrl} target="_blank" rel="noopener noreferrer"><Icon name="external" size={15}/>{C.source}: {item.sourceLabel}</a><span>{C.checked}: {item.verified}</span></div>
    <div className="smart-deadline-actions"><a className="btn btn-secondary smart-calendar-btn" href={calendarHref(item)} download={`auslanderleben-${item.id}.ics`}><Icon name="calendar" size={17}/>{C.calendar}</a><button className="btn smart-detail-btn" onClick={()=>dispatch({type:"OPEN_AREA_ITEM",id:item.areaItemId})}>{C.details} →</button></div>
  </article>;
}
