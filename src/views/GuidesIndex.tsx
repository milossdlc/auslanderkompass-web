...
const GUIDES:GuideCard[]=[
  ...
  {slug:"steuer-id-deutschland",de:"Steuer-ID nach der Anmeldung",en:"German tax ID after registration",group:"alltag",descDe:"Wann sie kommt, wo du sie findest und was du bei fehlendem Brief tun kannst.",descEn:"When it arrives, where to find it and what to do if the letter is missing."},
  {slug:"rundfunkbeitrag-umzug",de:"Rundfunkbeitrag nach Umzug",en:"Rundfunkbeitrag after moving",group:"wohnen",descDe:"Adresse ändern, Wohnung anmelden oder bei Zusammenzug abmelden.",descEn:"Update your address, register a home or deregister when moving in together."}
];
...
export function GuidesIndex({lang}:{lang:Lang}){
  ...
  return <div className="public-guide-shell">
    ...
    <main className="public-guide public-guides-index">
      <header className="public-guide-hero"><div className="eyebrow">Ausländerleben Guides</div><h1 className="disp">{de?"Deutschland verstehen – Schritt für Schritt":"Understand life in Germany – step by step"}</h1><p>{de?"Wähle ein Thema und erhalte eine klare Orientierung mit nächsten Schritten, typischen Unterlagen, häufigen Fehlern und offiziellen Quellen.":"Choose a topic and get clear orientation with next steps, common documents, common mistakes and official sources."}</p><div className="public-guide-trust"><span>{GUIDES.length} {de?"Guides":"guides"}</span><span>{de?"Offizielle Quellen":"Official sources"}</span><span>{de?"Kostenlos":"Free"}</span></div></header>
