...
function situationCopy(lang: Lang): { eyebrow: string; title: string; intro: string; glossary: string; situations: Situation[] } {
  if (lang === "de") return { eyebrow:"Praktischer Wegweiser", title:"Was möchtest du erledigen?", intro:"Starte mit deiner Situation. Ausländerleben führt dich dann zu den nächsten Schritten.", glossary:"Begriffe einfach erklärt", situations:[
  ...
  if (lang === "sr") return { eyebrow:"Praktični vodič", title:"Šta želiš da uradiš?", intro:"Počni od svoje situacije. Ausländerleben te vodi do sledećih koraka.", glossary:"Nemački pojmovi jednostavno objašnjeni", situations:[
  ...
  return { eyebrow:"Practical guide", title:"What do you want to do?", intro:"Start with your situation. Ausländerleben takes you to the next steps.", glossary:"German terms, simply explained", situations:[
...
export function Objasnjeno({ state, dispatch }: { state: AppState; dispatch: Dispatch<Action> }) {
  ...
  return <><div className="screen knowledge-screen"><header className="knowledge-heading"><div className="eyebrow">{C.eyebrow}</div><h1 className="disp">{C.title}</h1><p>{C.intro}</p></header>
    <div className="situation-grid">{C.situations.map(item=><button key={item.title} className="situation-card" onClick={()=>dispatch({type:"OPEN_AREA_ITEM",id:item.areaItemId})}><span className="situation-icon"><Icon name={item.icon as any} size={20}/></span><span className="situation-copy"><strong>{item.title}</strong><small>{item.text}</small></span><Icon name="chevron" size={18}/></button>)}</div>
    <section className="glossary-section"><div className="knowledge-subheading"><div className="eyebrow">{C.glossary}</div><p>{S.glossaryPage.intro}</p></div>{groups.map(areaId=>{const terms=S.glossary.filter(g=>g.area===areaId);if(!terms.length)return null;const label=areaId?S.areas[areaId].name:S.glossaryPage.groupGeneral;return <div key={areaId??"general"} className="glossary-group"><div className="mono glossary-group-title">{label}</div><div className="glossary-grid">{terms.map(g=><div key={g.term} className="card glossary-card"><div className="mono glossary-term">{g.term}</div><div className="glossary-explain">{g.explain}</div><div className="glossary-institution"><Icon name="building" size={13}/><span>{g.institution}</span></div></div>)}</div></div>})}</section>
